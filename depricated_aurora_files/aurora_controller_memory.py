...
chat_history = []
long_term_memory_file = Path("long_term_memory.json")
...
def save_long_term_memory(summaries: List[str]):
    with open(long_term_memory_file, "w", encoding="utf-8") as f:
        json.dump(summaries, f, indent=2)

def await maybe_summarize_and_store():
    global _summary_last_update, _summary_cache

    now = time.time()
    if now - _summary_last_update > _summary_refresh_interval:
        try:
            recent_text = "\n".join([f"{s}: {m}" for s, m in chat_history[-10:]])
            summary = await summarize_recent_memory()
            if summary and summary != _summary_cache["text"]:
                memory_store = load_long_term_memory()
                memory_store.append(summary)
                save_long_term_memory(memory_store)

def store_chat_exchange(user_msg, ai_msg):
    chat_data = {"user": user_msg, "ai": ai_msg}
    try:
        ...
        print("⚠️ Failed to save chat memory:", e)
                _summary_cache["text"] = summary
        except Exception as e:
            print("⚠️ Memory summarization error:", e)
        _summary_last_update = now
